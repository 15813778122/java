...
public class XMLDemo {
    ...
    public void dom4jParseXML() throws DocumentException {
        ...
        Iterator<org.dom4j.Element> iterator = rootElement.elementIterator();//获取的应该是根节点的子节点所组成的迭代器 两个person
        ArrayList<Person> persons = new ArrayList<>();
        Person p = null;
        while(iterator.hasNext()){
            p = new Person();
            org.dom4j.Element e = iterator.next();//获取的是person Element
            p.setPersonid(e.attributeValue("personid"));
            Iterator<org.dom4j.Element> iterator1 = e.elementIterator();//获取的是person的子节点
    ...
    /**
     * DOM解析XML
     * 1.基于树型结构，通过解析器一次性把文档加载到内存中，所以会比较占用内存，额可以随机访问
     * 更加灵活，更适合在WEB开发中使用
     */
    @Test
    public void domParseXML() throws IOException, SAXException, ParserConfigurationException {
        ...
        for (int i = 0; i < personNodeList.getLength(); i++) {
            Node perosonNode = personNodeList.item(i);
            p = new Person();
            //获取节点的属性值
            String personid = perosonNode.getAttributes().getNamedItem("personid").getNodeValue();
            p.setPersonid(personid);
            //获取当前节点的所有子节点
            NodeList childNodes = perosonNode.getChildNodes();
        ...
        System.out.println("result:"+ Arrays.toString(persons.toArray()));
    ...
    public void saxParseXML() throws ParserConfigurationException, SAXException, IOException {
        ...
        List<Person> person = personHandler.getPerson();
        for (Person s: person){
